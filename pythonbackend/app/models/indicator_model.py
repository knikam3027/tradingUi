...
def calculate_latest_straddle_indicators(
    ...
    volumes = [candle.get("c_volume_total", 0) for candle in straddle_candles]
...
def to_five_min_candles(candles: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    ...
    for candle in candles or []:
        ...
        rounded_minute = math.floor(minute / 5) * 5
        key = f"{date_part} {hour:02d}:{rounded_minute:02d}"
        ...
        if key not in buckets:
            ...
            bucket = buckets[key]
            bucket["high"] = max(bucket["high"], candle["open"], candle["close"])
            bucket["low"] = min(bucket["low"], candle["open"], candle["close"])
